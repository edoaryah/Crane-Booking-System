...
namespace AspnetCoreMvcFull.Services
{
  public class ScheduleConflictService : IScheduleConflictService
  {
    ...
    public async Task<bool> IsBookingConflictAsync(int craneId, DateTime date, int shiftDefinitionId, int? excludeBookingId = null, string craneCode = null)
    {
      try
      {
        ...
        return await IsBookingConflictByTimeAsync(craneId, date, shiftDefinition.StartTime, shiftDefinition.EndTime, excludeBookingId, craneCode);
      }
      catch (Exception ex)
      {
    ...
    public async Task<bool> IsMaintenanceConflictAsync(int craneId, DateTime date, int shiftDefinitionId, int? excludeMaintenanceId = null, string craneCode = null)
    {
      try
      {
        // Dapatkan shift definition untuk mendapatkan rentang waktu
        ...
        // Gunakan pemeriksaan konflik berbasis waktu
        return await IsMaintenanceConflictByTimeAsync(craneId, date, shiftDefinition.StartTime, shiftDefinition.EndTime, excludeMaintenanceId, craneCode);
      }
      catch (Exception ex)
      {
    ...
    public async Task<bool> IsBookingConflictByTimeAsync(int craneId, DateTime date, TimeSpan startTime, TimeSpan endTime, int? excludeBookingId = null, string craneCode = null)
    {
      try
      {
        ...
        foreach (var dateToCheck in datesToCheck)
        {
          ...
          // ✅ CONFLICT DETECTION: Exclude Cancelled dan Done
          var query = _context.BookingShifts
              .Include(rs => rs.Booking)
              .Where(rs => rs.Date.Date == dateToCheck &&
                      rs.Booking != null &&
                      rs.Booking.Status != AspnetCoreMvcFull.Models.BookingStatus.Cancelled &&
                      rs.Booking.Status != AspnetCoreMvcFull.Models.BookingStatus.Done &&
                      ((rs.Booking.CraneId == craneId) ||
                       (rs.Booking.CraneId == null &&
                        !string.IsNullOrEmpty(craneCode) &&
                        rs.Booking.CraneCode == craneCode)));
    ...
    public async Task<bool> IsMaintenanceConflictByTimeAsync(int craneId, DateTime date, TimeSpan startTime, TimeSpan endTime, int? excludeMaintenanceId = null, string craneCode = null)
    {
      try
      {
        // Normalisasi rentang waktu baru
        ...
        foreach (var dateToCheck in datesToCheck)
        {
          ...
          var query = _context.MaintenanceScheduleShifts
              ...
              .Where(ms => ms.Date.Date == dateToCheck &&
                      ((ms.MaintenanceSchedule!.CraneId == craneId) ||
                       (ms.MaintenanceSchedule!.CraneId == null &&
                        !string.IsNullOrEmpty(craneCode) &&
                        ms.MaintenanceSchedule.CraneCode == craneCode)));
