/**
 * MaintenanceHistory module - Complete Implementation
 * Diperbaiki sesuai dokumentasi pdfMake yang benar
...
var MaintenanceHistory = (function () {
  ...
  var config = {
    ...
    getTableUrl: '',
    exportTitle: 'Riwayat Pemeliharaan Crane',
    companyName: 'PT. KALTIM PRIMA COAL',
    currentUser: 'System', // Tambahkan ini
  ...
  function createDocumentDefinition(tableData) {
    ...
    var docDefinition = {
      ...
      info: {
        ...
        subject: 'Laporan Riwayat Pemeliharaan Crane',
      ...
      content: [
        ...
        // Main table dengan layout zebra tanpa border bold
        {
          ...
          table: {
            headerRows: 1,
            // Table widths sesuai dokumentasi pdfMake
            widths: [30, 'auto', '*', '*', 80, '*'],
            body: tableData
          },
          layout: {
      ...
      // Default style untuk seluruh dokumen
      defaultStyle: {
        fontSize: 9,
        color: '#2c3e50'
      },
  ...
  function processTableDataForPDF() {
    var tableData = [];
    var $table = $('#maintenanceHistoryTable');
    ...
    $table.find('thead th').each(function (index) {
      ...
      // Skip action column (biasanya kolom terakhir)
    ...
    pdfBtn.on('click', function (e) {
      ...
      if (dataTable) {
        try {
          ...
          dataTable.button(1).trigger(); // PDF (sekarang index 1 karena CSV dihapus)
        } catch (error) {
  ...
  return {
    ...
    init: function (options) {
      ...
      console.log('MaintenanceHistory initialized successfully');
    },
    ...
    testPDFGeneration: function () {
      try {
        ...
        if (window.pdfMake) {
          window.pdfMake.createPdf(docDef).download('test-maintenance-history.pdf');
        } else {
